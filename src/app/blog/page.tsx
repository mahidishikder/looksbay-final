      ...
      <section className="relative overflow-hidden bg-white px-6 pb-20 pt-36 sm:px-10 lg:px-16 lg:pb-24 border-b border-[#EAEAEA]">
        ...
        <div className="relative mx-auto max-w-[1640px]">
          <div className="mx-auto max-w-4xl text-center">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#0D0D0D] bg-white px-4 py-1.5 text-xs font-semibold text-[#0D0D0D] shadow-xs">
              <span className="w-2 h-2 rounded-full bg-[#F4511E] animate-pulse" />
              <span>Looksbay Studio Journal &amp; Insights</span>
            </div>
            <h1 className="mx-auto max-w-4xl text-4xl font-black leading-[1.06] tracking-tight sm:text-6xl lg:text-7xl text-[#0D0D0D]">
              Better websites start with <br />
              <span className="text-[#F4511E]">better thinking.</span>
            </h1>
              ...
              <a
                ...
                className={`rounded-full border px-4 py-2 text-xs font-bold transition-all shadow-xs ${
                  index === 0
                    ...
                    : "border-[#E5E5E5] bg-white text-[#555555] hover:border-[#0D0D0D] hover:text-[#0D0D0D]"
                }`}
              >
                {category}
              </a>
      ...
      <main id="latest" className="mx-auto max-w-[1640px] px-6 py-16 sm:px-10 lg:px-16 lg:py-24">
        ...
        <section aria-labelledby="featured-heading">
          <div className="mb-8 flex items-end justify-between gap-6">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#F4511E]">Featured Insight</p>
              <h2 id="featured-heading" className="mt-1 text-3xl font-black tracking-tight text-[#0D0D0D] sm:text-4xl">The latest thinking</h2>
            </div>
            <span className="hidden text-sm font-semibold text-[#888888] sm:block">Ideas for ambitious brands</span>
          </div>

          <article className="grid overflow-hidden rounded-[32px] border border-[#E5E5E5] bg-white shadow-xs hover:border-[#F4511E] hover:shadow-[0_20px_50px_rgba(244,81,30,0.08)] transition-all duration-300 lg:grid-cols-[1.08fr_0.92fr]">
            <Link href={`/blog/${featuredArticle.slug}`} className="group relative min-h-[300px] overflow-hidden lg:min-h-[520px]" aria-label={`Read ${featuredArticle.title}`}>
              ...
              <span className="absolute bottom-6 left-6 rounded-full bg-[#F4511E] px-3.5 py-1 text-[11px] font-bold uppercase tracking-wider text-white shadow-sm">Featured</span>
            </Link>
            <div className="flex flex-col justify-center p-7 sm:p-10 lg:p-14">
              <div className="flex items-center gap-3 text-xs font-bold uppercase tracking-wider text-[#F4511E]">
                <span>{featuredArticle.category}</span>
                <span className="h-1 w-1 rounded-full bg-[#CCCCCC]" />
                <span>{featuredArticle.readTime}</span>
              </div>
              <h3 className="mt-4 text-2xl font-black leading-tight tracking-tight text-[#0D0D0D] sm:text-3xl lg:text-4xl">
              ...
              <div className="mt-8 flex items-center justify-between gap-4 border-t border-[#F0F0F0] pt-6">
                ...
                <Link href={`/blog/${featuredArticle.slug}`} className="inline-flex items-center gap-2 text-xs sm:text-sm font-bold text-[#0D0D0D] hover:text-[#F4511E] transition-colors">
                  <span>Read full article</span>
                  <ArrowRight className="h-4 w-4 text-[#F4511E]" />
                </Link>
              </div>
            </div>
          </article>
        ...
        <section className="mt-20" aria-labelledby="all-insights-heading">
          <div className="mb-8 flex items-end justify-between gap-6">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#F4511E]">Explore Catalog</p>
              <h2 id="all-insights-heading" className="mt-1 text-3xl font-black tracking-tight text-[#0D0D0D] sm:text-4xl">More from the studio</h2>
            </div>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {supportingArticles.map((article) => (
              <article
                ...
                key={article.slug}
                className="group overflow-hidden rounded-3xl border border-[#E5E5E5] bg-white transition-all duration-300 hover:-translate-y-1 hover:border-[#F4511E] hover:shadow-[0_16px_40px_rgba(244,81,30,0.08)] shadow-xs flex flex-col justify-between"
              >
                <Link href={`/blog/${article.slug}`} className="block" aria-label={`Read ${article.title}`}>
                  ...
                  <div className="p-6">
                    <div className="flex items-center justify-between gap-3 text-[11px] font-bold uppercase tracking-wider text-[#F4511E]">
                      <span>{article.category}</span>
                      <span className="text-[#888888]">{formatDate(article.date)}</span>
                    </div>
                    <h3 className="mt-3 text-lg font-bold leading-snug tracking-tight text-[#0D0D0D] group-hover:text-[#F4511E] transition-colors">
                      {article.title}
                    </h3>
                    ...
                    <div className="mt-5 flex items-center gap-2 text-xs font-semibold text-[#888888]">
                      <Clock3 className="h-3.5 w-3.5 text-[#F4511E]" />
